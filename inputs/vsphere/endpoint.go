...
type queryChunk []gtypes.PerfQuerySpec
...
func (e *Endpoint) getDatacenterName(ctx context.Context, client *Client, cache map[string]string, r gtypes.ManagedObjectReference) (string, bool) {
	return e.getAncestorName(ctx, client, "Datacenter", cache, r)
}

func (e *Endpoint) getAncestorName(ctx context.Context, client *Client, resourceType string, cache map[string]string, r gtypes.ManagedObjectReference) (string, bool) {
	path := make([]string, 0)
	returnVal := ""
	here := r
	done := false
	...
	for _, r := range resources {
		...
		if e.customAttrEnabled {
			for _, cv := range r.Summary.CustomValue {
				val := cv.(*gtypes.CustomFieldStringValue)
				if val.Value == "" {
					continue
				}
...
func (e *Endpoint) chunkify(ctx context.Context, res *resourceKind, now time.Time, latest time.Time, job queryJob) {
	...
	for _, obj := range res.objects {
		timeBuckets := make(map[int64]*gtypes.PerfQuerySpec)
		for metricIdx, metric := range res.metrics {
			...
			if !ok {
				bucket = &gtypes.PerfQuerySpec{
					Entity:     obj.ref,
					MaxSample:  maxSampleConst,
					MetricId:   make([]gtypes.PerfMetricId, 0),
...
func (e *Endpoint) alignSamples(info []gtypes.PerfSampleInfo, values []int64, interval time.Duration) ([]gtypes.PerfSampleInfo, []float64) {
	rInfo := make([]gtypes.PerfSampleInfo, 0, len(info))
	...
	for idx := range info {
		...
		if roundedTs == lastBucket {
			...
			roundedInfo := gtypes.PerfSampleInfo{
				Timestamp: roundedTs,
