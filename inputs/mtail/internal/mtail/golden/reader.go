...
func ReadTestData(file io.Reader, programfile string) metrics.MetricSlice {
	...
	for scanner.Scan() {
		...
		if match[4] != "" {
			...
			// log.Printf("type is %q", typ)
		}
		var timestamp time.Time
		// log.Printf("match 5: %q\n", match[5])
		if match[5] != "" {
		...
		// log.Printf("timestamp is %s which is %v in unix", timestamp.Format(time.RFC3339), timestamp.Unix())
