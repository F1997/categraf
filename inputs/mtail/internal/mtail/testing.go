...
import (
	...
	"github.com/F1997/categraf/inputs/mtail/internal/waker"
)
...
func (ts *TestServer) PollWatched(n int) {
	log.Println("Testserver starting poll")
	log.Println("TestServer polling filesystem patterns")
	if err := ts.t.Poll(); err != nil {
		log.Println(err)
	}
	log.Println("TestServer reloading programs")
	if err := ts.r.LoadAllPrograms(); err != nil {
		log.Println(err)
	}
	log.Println("TestServer tailer gcing")
	if err := ts.t.ExpireStaleLogstreams(); err != nil {
		log.Println(err)
	}
	log.Println("TestServer waking idle routines")
	ts.awaken(n)
	log.Println("Testserver finishing poll")
}
