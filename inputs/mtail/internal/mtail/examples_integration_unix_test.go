...
import (
	...
	"github.com/F1997/categraf/inputs/mtail/internal/mtail"
		...
		t.Run(fmt.Sprintf("%s on %s", tc.programfile, tc.logfile),
			testutil.TimeoutTest(exampleTimeout, func(t *testing.T) { //nolint:thelper
				...
				go func() {
					...
					testutil.FatalIfErr(t, err)
					log.Printf("Copied %d bytes into pipe", n)
					source.Close()
					pipe.Close()
				}()
			...
			t.Run(fmt.Sprintf("%s on %s://%s", tc.programfile, scheme, tc.logfile),
				testutil.TimeoutTest(exampleTimeout, func(t *testing.T) { //nolint:thelper
					...
					go func() {
						...
						testutil.FatalIfErr(t, err)
						log.Printf("Copied %d bytes into socket", n)
						if scheme == "unixgram" {
							// Write zero bytes after Stop is called to signal that this is the "end of the stream".
							for {
								_, err = s.Write([]byte{})
								if err == nil {
									log.Printf("Zero bytes written to socket to signal EOF")
									break
								}
